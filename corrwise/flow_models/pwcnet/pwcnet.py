...
# TURN THIS OFF, WE NEED THE BACKWARD PASS
...
class Network(torch.nn.Module):
	def __init__(self, default=True):
		...
		if default:
			state_dict_name = 'pwcnet.pth'
		else:
			state_dict_name = 'pwcnet-things.pth'

		self.load_state_dict({ strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in torch.load(__file__.replace('pwcnet.py', state_dict_name)).items() })
	# end
...
def estimate(tenFirst, tenSecond, netNetwork):
	...
	#tenPreprocessedFirst = tenFirst.cuda().view(num_batch, 3, intHeight, intWidth)
	#tenPreprocessedSecond = tenSecond.cuda().view(num_batch, 3, intHeight, intWidth)
	tenPreprocessedFirst = tenFirst.view(num_batch, 3, intHeight, intWidth)
	tenPreprocessedSecond = tenSecond.view(num_batch, 3, intHeight, intWidth)
